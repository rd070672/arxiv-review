...
固体冷却の有力な候補として注目されているのが、**電気熱量効果**（electrocaloric effect: ECE）を利用する手法である。ECE とは、強誘電体や誘電体に電場を印加・除去する操作によって材料が断熱的に温度変化する現象であり、いわば「電場で制御できる熱力学サイクル」を材料の内部に実現するものだ。電場を加えると双極子がそろって配列し分極が増大するとともに温度が上昇し（熱浴に放熱）、電場を取り除くと双極子が乱れ温度が低下する（熱浴から吸熱）——このサイクルを繰り返すことで連続的な冷却効果を得る。圧縮機を必要とせず、駆動部品がないため機械振動もなく、デバイスの小型化・集積化にも適している。特に半導体チップの局所冷却（ムーアの法則が限界に達しつつある現代の演算チップの発熱問題）や、携帯型医療機器・IoT センサーなど小型デバイスへの応用が期待されており、近年は研究が急速に加速している。同種のカロリック効果として、磁場を使う**磁気熱量効果**（magnetocaloric effect: MCE）や機械応力を使う**弾性熱量効果**（elastocaloric effect: eCE）もあるが、ECE は電場制御という点でシリコン半導体プロセスとの親和性が高く、集積化デバイスへの応用可能性が最も高いと期待されている。
...
### パーコレーションモデルとフラストレーションモデルの対比

ECE の物理メカニズムを理解する上で、Jiang らの PMN リラクサーシミュレーション（arXiv:1802.01784）と Li ら（2602.21520）のモデルを比較することは重要な視点を与える。Jiang らが示した「PNR のパーコレーション」描像では、孤立した極性ナノ領域が電場印加によって互いに連結し、巨大なマクロドメインへと成長する相転移的プロセスが ECE の起源とされた。この過程は本質的に一種の臨界現象であり、パーコレーション閾値近傍で ECE が最大化されると予測される。

これに対し Li らのモデルでは、フラストレーションを受けた高分極ナノドメインが「段階的・非同時的に」電場に応答するという描像が提案されている。すなわち、個々のナノドメインが各自の「応答しきい電場」を持ち、それらが温度・組成揺らぎによって広く分布することで、全体として広い温度域にわたるブロードな ECE 応答が実現されると解釈される。この二つのモデルは相互排他的ではなく、むしろ「PMN 型パーコレーション」をフラストレーション系で実現した場合に何が変わるのか——閾値がブロード化し段階的パーコレーションが起こる——という統合描像が考えられる。この点の実験的検証には、電場印加下でのリアルタイム中性子全散乱（PDF の電場・温度同時依存性）が最も直接的なアプローチとなるだろう。

多結晶セラミックスという観点からも重要な問題がある。Hsu ら（arXiv:2402.00777）の計算が示したように、電場方向は BaTiO₃ の ECE の大きさと符号を劇的に変化させる。多結晶体では各結晶粒が異なる方向を向いているため、印加電場に対してそれぞれ異なる ECE 応答を示す。NNO-BTH 系においても、個々の結晶粒の配向が ECE 発現に寄与する効率は均一ではない。テクスチャー制御（配向多結晶や単結晶化）によってどの程度 ECE が向上するかは、未検討のままである。NaNbO₃ 系単結晶を育成して電場方向依存性を体系的に測定することは、フラストレーション戦略の理論的上限を評価する上で価値ある実験となるだろう。
...
**マルチカロリック効果**（multicaloric effect）も注目分野として台頭している。磁場と電場、あるいは応力と電場を組み合わせることで ECE を増強または制御する概念であり、Fe₄₈Rh₅₂ のような磁気的・弾性的・電気的カロリック効果を合わせ持つ材料でその有効性が実証されている。NaNbO₃ 系においても、圧力誘起相転移と ECE を組み合わせる「マルチカロリック」応用の可能性が理論的に示唆されている。弾性熱量効果の分野では、形状記憶 Heusler 合金における巨大 eCE の実証（本誌の別記事 2026-04 参照）が同時期に進んでいるが、電気的駆動 vs. 機械的駆動という観点では ECE がデバイス集積化で優位であり、両者は相補的な応用領域を持つ。
...
ECE 材料の組成空間は広大であり、多元素ペロブスカイトの全ての組み合わせを実験で網羅することは不可能である。近年、第一原理計算・フェーズフィールド法・機械学習ポテンシャルを組み合わせた「高スループット計算スクリーニング」が ECE 材料探索にも適用され始めている。例えば、(K,Na)NbO₃ 系のフェーズフィールドモデルを用いた ECE 特性の系統的予測や、有効ハミルトニアンに基づく PMN 系リラクサーの相図計算などが報告されている。Li ら（2602.21520）もフェーズフィールドシミュレーションを設計指針として用いているが、今後は機械学習によって「極性フラストレーションが最大となる組成」を自動探索するアプローチが有力になると考えられる。また、FNLC（arXiv:2510.26386）において著者らが 100 種以上の材料を同定できたことも、組成空間の系統的スクリーニングなしには困難であった。材料インフォマティクスと ECE 材料研究の融合は、今後の最重要テーマの一つとなるだろう。
...
ECE 材料の機能発現には局所構造が本質的な役割を果たすため、局所構造の直接観察は極めて重要である。通常の X 線回折や中性子回折による「平均構造」解析だけでは、リラクサーで重要な nm スケールの局所変位分布を捉えられない。ここで威力を発揮するのが**対分布関数**（pair distribution function: PDF）解析である。PDF は全散乱（Bragg 散乱＋散漫散乱）の実験データをフーリエ変換して得られ、「ある原子からの距離 *r* に別の原子が存在する確率密度 G(r)」を与える。この量は原子間距離の分布に敏感であり、例えば酸素のランダム変位やナノドメイン内での局所格子歪みを定量化できる。さらに Li ら（2602.21520）が用いた **3D big-box モデリング**では、数千原子規模の計算セルを使い、観測された PDF とともに散漫散乱パターン全体を同時再現する構造モデルを構築する。これにより、酸素がどの方向にどれだけの確率で変位しているかという 3 次元確率マップが得られ、BTH ドーピングによる酸素傾斜フラストレーションの定量化が可能となった。この多点測定（STEM/2D ＋ 中性子全散乱/3D）の組み合わせが、本研究の証拠の強さを支えている。

### 性能指数（FOM）の定義

ECE 材料を比較するための指標としていくつかの FOM が提案されているが、最も直接的なものの一つが **ΔT × ΔTspan** である。大きな ΔT を達成してもそれが狭い温度域でのみ発現するなら、実用的なカスケード型デバイスに組み込みにくい。逆に広い温度域で ECE が発現しても ΔT が微小では冷却効果が弱い。この FOM はその両立性を評価する指標として Li らが用いており、90 K² 超という達成値は同等条件の鉛フリーセラミックスでは報告例が少ない。その他の FOM としては、単位質量あたりの冷凍能力（refrigerant capacity: RC = ∫|ΔT| dT を ΔTspan の温度範囲で積分した量）や、Carnot 効率比（COP_material / COP_Carnot）、耐サイクル性（cycle life）なども重要な評価指標である。これらの指標の間には必ずしも相関がないため、目的とするデバイス性能に応じて適切な指標を選ぶ必要があり、報告値を文献間で比較する際には使用 FOM の定義を確認することが不可欠である。
...
今回の注目論文（arXiv:2602.21520）が拓いた視界は、「極性フラストレーション戦略」という明快な設計指針で鉛フリーセラミックスにおける ΔT と ΔTspan のトレードオフを同時に克服できることを示した点にある。BTH を「極性レンチ」として機能させ酸素八面体傾斜秩序を意図的に撹乱する——このアプローチは NaNbO₃ 系の枠を超えて、酸素傾斜が重要な役割を果たす他の鉛フリーペロブスカイト（BaTiO₃ 系、KNbO₃ 系など）にも転用可能な原理である可能性がある。また Du ら（Nature 2025）の高極性エントロピー戦略と合わせて考えると、「局所的な極性配置の多様性を意図的に創出する」という設計思想が次世代 ECE 材料の共通基盤として確立されつつあると言える。2402.00777 が示した電場方向制御や 2403.18475 の Mott 転移利用、2510.26386 の液晶 ECE という多様なアプローチが並立して議論されていることは、ECE 材料研究が材料科学・凝縮系物理・物性化学にまたがる総合的な分野として成熟しつつあることの証左である。現在かなり確からしいと言えるのは、（1）適度な化学的・構造的無秩序の導入が ECE の ΔTspan 拡大に有効であること、（2）鉛フリーセラミックスでも FOM > 90 K² が到達可能であること、（3）ナノスケール ECE の局所測定が実現しつつあること、の三点である。

一方で、多くの本質的な問いが依然として未解決である。フラストレーテッドナノドメインの「動的挙動」——電場印加下での成長速度、隣接ドメインとのカップリング、繰り返しサイクルによる疲労——は実験的にほとんど検証されていない。間接 ECE 測定と直接 ECE 測定の体系的な乖離問題も、材料横断的な評価プロトコルが確立されていない現状では解決されていない。また、フラストレーション量（どの程度の酸素傾斜無秩序が最適か）の定量指標は Li ら（2602.21520）においても経験的に評価されており、理論的な最適化設計には至っていない。今後 1〜3 年で最も重要な展開となりうるのは、第一に SEcT のようなナノスケール ECE 観察とフェーズフィールドシミュレーションの融合による「ナノドメイン—ECE 対応関係の定式化」、第二に中性子散乱・X 線散漫散乱を活用した「電場印加下のリアルタイム局所構造観察」（放射光・中性子の時間分解実験）、そして第三に「ΔT > 2 K かつ ΔTspan > 100 K を室温付近・低電場（< 40 kV/cm）・鉛フリー・高サイクル耐久性条件下で同時達成する」材料の実証——この三点であろう。ECE 研究は今まさに基礎物理の理解から応用デバイスへの橋渡しを迫られる段階に入っており、材料インフォマティクスと組み合わせた加速的材料探索が鍵を握るであろう。
...
9. **Zheng Jiang, Yonghao An, Xianzhe Chen, Binhan Gu, Hong Liu, Biao-Liang Feng, Ce-Wen Nan, Long-Qing Chen**, "Electrocaloric effect of a lead-free relaxor ferroelectric under electrical field: An atomistic simulation study," *arXiv:1802.01784* (2018).
   [https://arxiv.org/abs/1802.01784](https://arxiv.org/abs/1802.01784)
   【背景・理論】有効ハミルトニアンを用いた原子論的シミュレーションにより、PMN 系リラクサー強誘電体における PNR のパーコレーションが ECE の起源であることを計算で示した重要な理論研究。フラストレーションモデルとの対比に不可欠な背景を提供。

10. **Aiden Ross, Mikael Sondergaard, Sean P. Hau-Riege, Fionn Rodgers, Ari Paavo Oja, James Mansell, Veit Elser, Bea Noheda, Graeme Blake**, "Thermodynamic Model for the Ferroelectric Transitions of KNbO₃," *arXiv:2601.17150* (2026).
    [https://arxiv.org/abs/2601.17150](https://arxiv.org/abs/2601.17150)
    【波及・関連材料】KNbO₃ の強誘電相転移の熱力学を統一的フレームワークで記述した研究。BaTiO₃ と同型のペロブスカイト強誘電体への極性フラストレーション戦略の展開可能性を示す関連材料系の基礎研究。
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               
