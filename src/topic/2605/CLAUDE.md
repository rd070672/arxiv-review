...
なぜこの問題が重要か
この分野で何が課題になっているのかを説明する。
材料物性、計測技術、デバイス応用、社会的要請の観点から、なぜ今この研究が必要なのかを示す。
...
何がまだ分かっていないのか
論文が取り組む本質的な問いを整理する。
単なる技術的課題ではなく、「何を理解すれば分野が進むのか」を明確にする。
...
何をして、何を示したのか
対象論文の主張、方法、結果を中立的に整理する。
ここでは過度な評価をせず、「論文そのものの内容」を正確にまとめる。
...
この論文は何を変えたのか
関連論文と比較しながら、この論文の位置づけを明確にする。
「以前から分かっていたこと」「今回新しく分かったこと」「まだ仮説に留まること」を分けて書く。
...
どの解釈が最も妥当か
レビューの中心部分。
注目論文の解釈をそのまま受け入れるのではなく、根拠の強さ、別解釈、残された不確実性を検討する。
...
まだ弱い主張・過剰解釈の可能性
追加で必要な実験・解析
※ここでは「論文の要約」ではなく、「論文の主張をどこまで信じてよいか」を議論する。

## 6. 何が一般化できるか

設計原理へ拡張できるか
この章では、注目論文の知見が特定の材料や条件に限られるのか、それともより広い材料設計・測定原理・理論枠組みに拡張できるのかを検討する。
...
一般化できる結論
一般化にはまだ慎重であるべき点
※9章の将来展望と重複しないように、ここでは「何が広く使える知見か」に集中する。

## 7. 基礎から理解する

このトピックを理解するための前提知識
この章では、学部4年生が理解できるように、今回の論文を読むために必要な基礎概念を丁寧に説明する。
いきなり論文の詳細に入らず、物理現象、材料組織、測定原理、解析モデルなどを段階的に解説する。
...
論文中で使われる代表的な式や物理量
各記号の意味、式の成立条件、近似の範囲
それらが今回の論文の主題とどうつながるか
※この章は全体の約4分の1程度の文量を使い、初学者が論文の核心に入るための足場となるように書く。
必要に応じて数式を用いて説明することを推奨する
数式における各記号の意味、各項目が何を表すか、どの近似で成り立つか、このトピックとどうつながるかを明記すること。
...
次に何をすべきか
この章では、レビュー全体を踏まえて、今後の研究の方向性を3段落程度で整理する。
単なるまとめではなく、「次に何を検証すれば分野が進むのか」を明確にする。

書く内容

第1段落では、現時点でかなり確からしくなったことを整理する。
第2段落では、まだ未確定な点や、解釈が分かれる点を整理する。
第3段落では、次に重要となる実験・理論・計算・データ解析、および今後注目すべき論点を述べる。
※7章の「一般化」と重複しないように、ここでは「今後やるべき研究」に焦点を当てる。
