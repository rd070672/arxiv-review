...
CLAUDE.md の指示に従い、arXiv cond-mat recent を起点として最近注目される話題を1つ選び、日本語で学部4年生向けの「図入り特集解説記事」を Markdown で作成すること。その際、reported_papers.csv を全て参照し、過去のanchor 論文は全て除外するようにしてください。

--------------------------------
## タスクの基本方針
--------------------------------
本タスクでは、1本の注目論文を記事全体の主軸に据え、その論文が提起した問い・示した新規性・残した未解決点を中心に、関連論文5〜10本を有機的に織り込みながら、1つのトピックを深く考察する総合レビュー記事を構成すること。パラグラフライティングを意識してください。
...
--------------------------------
選定時は以下を考慮すること。
- reported_papers.csv を全て参照し、anchor 論文は原則除外する
- 直近 10 本の履歴から main-area の偏りを確認する

https://arxiv.org/list/cond-mat/recent?skip=0&show=250
にアクセスして、過去1週間の arXiv 新着を確認し、その中から毎回 html 形式のあるプレプリントを 5〜8 本候補化すること。
Claude in chrome は使わず、上記リンクから web 検索すること。

候補が決まったら、悩まずに過去のアンカー論文と一番遠い領域と思われる論文を即座に選択するようにしてください。
...
先に記事を作成して、必要に応じて、適切な図面をダウンロードするようにしてください。
図面は積極的に使うようにしてください。
...
--------------------------------
# トピック名
（簡潔で内容がわかる題目にしてください。インパクトのある題目は控え、短めのタイトルにして）

- 執筆日
- 要旨（超簡潔にレビュー内容を日本語でまとめてください）
...
## 2. 解決すべき課題
この分野における本質的な問いをいくつかに整理し、どこに議論の分岐点があるのかを示すこと。
それぞれについて段落ごと記述し、より深い物理的、技術的問いに昇華して議論してください。
解決方策も検討してそのアプローチや仮説などについても触れてください。

## 3. 注目論文の仮説と成果
注目論文の主張、方法、主要結果、新規性を丁寧に説明する。
特に、注目論文の内容を丁寧に説明する。
- 研究目的、対象材料・対象現象
- 研究アプローチ（実験・計算方法、解析手法）
- 主要結果
- 論文の新規性と限界
ここでは、注目論文の要約だけで終わらせない。
どの図、どのデータ、どの解析が結論を支えているのかを具体的に説明する。
それ以前に何が分かっていたか、今回どこが前進したか、何がまだ仮説段階かを区別して書くこと。

## 4. 研究史における位置づけ
関連論文を使いながら、背景となる研究史、近年の流れ、複数の立場や解釈を整理する。
関連技術や知見の系譜をたどりながら注目論文の技術を俯瞰し整理する。
その中で、この論文はどこに位置づくか記述する。

## 5. どの解釈が最も妥当か
関連論文を横断的に参照しながら、2.未解決問題への問いに対してどんな仮説が出てくるか整理する。
2番と表現が重複しないように整理する。
- 注目論文の解釈を支える根拠や、他の可能な解釈や仮説
- 実験・理論・計算の整合とずれ
- 比較的強く支持される結論や、まだ弱い結論
- 今後必要な検証
を掘り下げて整理すること。
...
今回のトピックの下となる基礎的な概念を説明する上で、知っていくべき基本的な内容から丁寧に段階的に十分な文量で説明してください。
基礎の基礎から説明する用に、定義やその説明の中に出てくる専門的な用語もわかるように、多めの文量を確保して基本の基本から説明してください。
...
## 9. 将来展望
要約だけで終わらせず、今かなり確からしくなったこと、まだ未確定なこと、次に重要になる実験・理論・計算と今後注目すべき論点
に関連する内容を将来展望として記述して、パラグラフライティングして、3段落程度で記述すること。
...
- 用語も数式もボールド`**`にしないようにしてください
- もし`**~(~)**`とする場合は、後ろの**のあとに半角スペースを置くように`**~(~)** `と記述する
...
- 各節は十分に説明的に書いてよい
- 数式は理解を助けるので、可能な限り沢山記述する
- 数式の各項や各記号の意味は必ず説明すること
- 図は少なすぎず、論点理解に寄与するものを選ぶこと
