...
Claude in chromeは使わずに上記リンクからweb検索するようにして下さい。
にアクセスして、過去1週間の arXiv 新着を確認し、その中から毎回html形式のあるプレプリント5~10本を選定する。
- スケジュールプロンプトで指定された reported_papers.csv を参照し、anchor論文は原則除外する
- 直近8〜12本の履歴から main-area と sub-area の偏りを確認する
- 同程度なら最近少ない main-area を優先する
- 直近5回で3回以上出た sub-area は原則回避する（キーワードを指定している場合はその限りでない）
...
- 該当フォルダ内にある、reported_papers.csv を参照し、同じ論文を選択するのは極力避ける
- figures内が重複するのは問題ないので確認は不要
- ある程度候補が決まったら悩まずにパッと選んでしまってよい。
- その分野で中心的な問いをよく表している
...
## 分量の目安
--------------------------------
- 記事本文は丁寧さを保ち、全体で 16,000~18,000 字程度を目安とする
- 多少多くても少なくても問題なく、書き直す必要はない
- 各節は十分に説明的に書いてよい
...
##終了後
最後の報告のためのClaudeチャット内での出力は不要なので、完了したことのみを教えてください。
